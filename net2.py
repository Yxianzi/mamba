...
import torch.nn as nn
from mamba_ssm import Mamba  # 确保已安装 mamba-ssm

class BiSSMBlock(nn.Module):
    """双向 Mamba 块，解决高光谱波段长距离依赖，复杂度为线性 O(N)"""
...
class DCRN_Mamba(nn.Module):
    ...
    def __init__(self, input_channels, patch_size, n_classes):
        ...
        # 替换原有的 self.atten = CrossAttention(...)
        self.mamba_backbone = BiSSMBlock(d_model=288)  # 288 是原代码池化后的特征维度
        print("2个head")
    ...
    def forward(self, x,y):
        ...
        x = torch.cat((x1, x2), 1)  # (Batch, 288, 7, 7)
        y = torch.cat((y1, y2), 1)  # (Batch, 288, 7, 7)

        # 2. 空间-通道注意力增强 (保持 MLUDA 的多级对齐特性)
        x = self.ca(x) * self.sa(x) * x
        y = self.ca(y) * self.sa(y) * y

        # 3. 池化并展平
        x = self.avgpool(x).view(x.shape[0], -1)  # 结果维度: (Batch, 288)
        y = self.avgpool(y).view(y.shape[0], -1)  # 结果维度: (Batch, 288)

        # 4. 创新点：使用 Bi-SSM (Mamba) 替换 CrossAttention
        # Mamba 需要序列输入 (Batch, Length, Dim)，这里将特征视为长度为 1 的序列
        feat_x = self.mamba_backbone(x.unsqueeze(1)).squeeze(1)  # (Batch, 288)
        feat_y = self.mamba_backbone(y.unsqueeze(1)).squeeze(1)  # (Batch, 288)
