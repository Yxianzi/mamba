...
def physical_aligned_mmd(s_feat, t_feat, delta_phys):
    """
    根据公式: Loss = || mu_target - (mu_source + delta_phi) ||^2
    delta_phys: 预设的物理偏移向量（如根据红边带平移规律设定的常数）
    """
    mu_s = torch.mean(s_feat, dim=0)
    mu_t = torch.mean(t_feat, dim=0)
    loss = torch.norm(mu_t - (mu_s + delta_phys), p=2)
    return loss
