...
from config_UP2PC import * # 导入 UP2PC 数据集的超参数
import os
...
##################################
# 0. 准备工作
if not os.path.exists('classificationMap/UP2PC/UDA_CETA'):
    os.makedirs('classificationMap/UP2PC/UDA_CETA')
...
for iDataSet in range(nDataSet):
    ...
    plt.title(f'Accuracy Metrics (CETA-Mamba UP2PC) - Run {iDataSet + 1}', fontsize=14)
    plt.legend(loc='lower right')
    plt.grid(True, linestyle='--', linewidth=0.5, alpha=0.7)
    plt.tight_layout()
    plt.savefig(f'classificationMap/UP2PC/UDA_CETA/accuracy_curve_run{iDataSet + 1}.png', dpi=300)
    plt.close()
    ...
    plt.title(f'Training Loss (CETA-Mamba UP2PC) - Run {iDataSet + 1}', fontsize=14)
    plt.legend(loc='upper right')
    plt.grid(True, linestyle='--', linewidth=0.5, alpha=0.7)
    plt.tight_layout()
    plt.savefig(f'classificationMap/UP2PC/UDA_CETA/loss_curve_run{iDataSet + 1}.png', dpi=300)
    plt.close()

    log_dir = 'classificationMap/UP2PC/UDA_CETA'
...
if nDataSet > 0:
    ...
    summary_content = [
        "\n" + "#" * 50,
        f"🎉 FINAL STATISTICAL RESULTS OVER {nDataSet} RUNS (UP2PC) 🎉",
        "#" * 50,
