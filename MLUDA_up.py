...
##################################
# 0. 准备工作：创建保存目录
if not os.path.exists('classificationMap/Pavia'):
    os.makedirs('classificationMap/Pavia')
...
print('Source shape:', data_s.shape, label_s.shape)
print('Target shape:', data_t.shape, label_t.shape)
...
for iDataSet in range(nDataSet):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i in range(1, num_iter):
            ...
            # 保持 UP2PC 特有的 Loss 权重组合 (LMMD 权重为 0.3)
            adapt_loss = 0.3 * lmmd_loss + contrastive_loss_t + contrastive_loss_s
    ...
    plt.title(f'Training Metrics (ETA-Mamba UP2PC) - Run {iDataSet + 1}')
    plt.grid(True, linestyle='--', linewidth=0.5)
    plt.tight_layout()
    plt.savefig(f'classificationMap/Pavia/metrics_curve_run{iDataSet + 1}.png', dpi=300)
    plt.close()

    log_dir = 'classificationMap/Pavia/'
