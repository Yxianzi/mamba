...
from config_SH2HZ import * # 导入上海-杭州数据集的超参数
import os
...
##################################
# 0. 准备工作
if not os.path.exists('classificationMap/SH2HZ/UDA_CETA'):
    os.makedirs('classificationMap/SH2HZ/UDA_CETA')

# 【注意】如果你的 utils 里读取 SH2HZ 的函数名不是 load_data_shanghai，请自行微调此处
...
for iDataSet in range(nDataSet):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i in range(num_iter):
            ...
            # ====================================================================
            # 🚀 原创 2: IE-SPA 宏观中心柔性对齐 (保留 pa_loss 版本)
            # ====================================================================
            if epoch > 20:
                # 【拯救方案】：只有 >0.9 的顶级自信样本才能更新目标原型
                mask_proto = max_probs_t > 0.9

                if mask_proto.sum().item() > 0:
                    proto_manager.update_target(target_features[mask_proto], pseudo_label_t[mask_proto])

                pa_loss_val = proto_manager.get_aligned_loss()
                pa_loss = 0.1 * torch.log1p(pa_loss_val)
            else:
    ...
    plt.plot(range(1, len(kappa_list) + 1), kappa_list, label='Kappa (x100)', color='orange', linestyle=':', linewidth=2)
    plt.xlabel('Epoch', fontsize=12)
    plt.ylabel('Score', fontsize=12)
    plt.title(f'Accuracy Metrics (CETA-Mamba SH2HZ) - Run {iDataSet + 1}', fontsize=14)
    plt.legend(loc='lower right')
    plt.grid(True, linestyle='--', linewidth=0.5, alpha=0.7)
    plt.tight_layout()
    plt.savefig(f'classificationMap/SH2HZ/UDA_CETA/accuracy_curve_run{iDataSet + 1}.png', dpi=300)
    plt.close()
    ...
    plt.title(f'Training Loss (CETA-Mamba SH2HZ) - Run {iDataSet + 1}', fontsize=14)
    plt.legend(loc='upper right')
    plt.grid(True, linestyle='--', linewidth=0.5, alpha=0.7)
    plt.tight_layout()
    plt.savefig(f'classificationMap/SH2HZ/UDA_CETA/loss_curve_run{iDataSet + 1}.png', dpi=300)
    plt.close()

    log_dir = 'classificationMap/SH2HZ/UDA_CETA'
